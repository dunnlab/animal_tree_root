...
taxa = 
	read_tsv("taxonomy_info/taxon_table.tsv") %>% 
...
phylip_file_names = list.files(pattern = "blast/phylips/.+\\.phy$")

a_file = "blast/phylips/Chang2015_Chang2015.phy"
p_file = "blast/phylips/Chang2015_Chang2015.nex"
msa = PartitionedMultipleAlignment( a_file, p_file, taxa )
...
ggplot() + 
	...
	geom_rect(data=D, mapping=aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, fill=clade )) + 
...
partition_clusters = 
	read_tsv( "blast/graphs/diamond_partition_clusters.tsv" ) %>% 
	dplyr::rename( partition=partition_name ) %>%
	dplyr::rename( matrix=dataset ) %>%
	dplyr::rename( cluster_id=cluster_number )

busco_results = 
	read_tsv("blast/graphs/busco_metazoa_results.tsv") %>% 
	filter( Status != "Missing" ) 

sequences_all_txt = 
	read_tsv("blast/all_parts_list.txt")

names(sequences_all_txt) = c("full_name")
sequences_all = str_split_fixed( sequences_all_txt$full_name, ":", 5 ) %>% 
	as_tibble()
names( sequences_all ) = c( "manuscript", "matrix", "species", "ncbi_taxon_id", "partition" )

partitions_all = 
	sequences_all %>% 
	group_by( manuscript, matrix, partition ) %>% 
...
Bs = 
	str_split_fixed( busco_results$Sequence, ":", 5 ) %>% 
	as_tibble()

names( Bs ) = c( "manuscript", "matrix", "species", "ncbi_taxon_id", "partition" )
...
busco_distinct =  busco_results %>% select( manuscript, matrix, partition, Description ) %>% distinct(  )
busco_overrepresented = busco_distinct %>% group_by( manuscript, matrix, partition ) %>% summarise( n = n() ) %>% filter( n > 1 )
...
partition_clusters %<>% left_join( partition_to_busco_map, by=c("manuscript", "matrix", "partition") )


busco_summary = 
	partition_to_busco_map %>% 
	group_by( manuscript, matrix ) %>%
...
matrix_summary =
	...
	left_join(
		busco_summary,
		by=c("manuscript", "matrix")
	)

ggplot( matrix_summary ) +
	geom_point( aes(x=n_partitions, y=n_busco_partitions, col=manuscript) ) + geom_abline(slope=1,intercept=0) + coord_fixed()
...
```{r taxon_mapping_across_studies}

Dtaxaxman = 
	read_tsv("~/Dropbox/repo/animal_root/blast/graphs/cross_mscript_taxa.tsv") %>%
...
Dtaxa = 
	read_tsv("~/Dropbox/repo/animal_root/blast/graphs/taxon_graph.tsv") %>%
	arrange( mscript1, dataset1, taxon1, mscript2, dataset2, taxon2 ) 
...
taxonomy_reference = read_tsv("taxonomy_info/taxon_table.tsv")
...
Dpart = 
	read_tsv("~/Dropbox/repo/animal_root/blast/graphs/partitions_graph.tsv") %>% 
	mutate( node1 = paste(mscript1, dataset1, part1, sep = ':') ) %>% 
	mutate( node2 = paste(mscript2, dataset2, part2, sep = ':') )

Dpart_self = Dpart %>% 
	filter( node1==node2 ) %>%
	select( mscript1, dataset1, part1, node1, count )
...
D_source = Dpart %>%
  distinct(node1, .keep_all = TRUE ) %>%
	select( node1, mscript1, dataset1, part1 ) %>%
  dplyr::rename(label = node1, mscript = mscript1, dataset = dataset1, part = part1 )

D_dest = Dpart %>%
  distinct(node2, .keep_all = TRUE ) %>%
	select( node2, mscript2, dataset2, part2 ) %>%
  dplyr::rename(label = node2, mscript = mscript2, dataset = dataset2, part = part2 )
...
#gene_network = network( edges, directed=FALSE, vertex.attr = nodes, matrix.type = "edgelist", ignore.eval = FALSE)
#
#gene_matrix = as.sociomatrix(gene_network)

#library( MCL )

#gene_clusters = mcl( x=gene_matrix, addLoops = TRUE )
...
# Investigate commencted components of genes (ie partitions) that include multiple members fromt he same matrix
D = nodes %>% group_by( mscript, dataset, cluster ) %>% summarise(n=n()) %>% arrange(desc(n))
D %>% print(n=100)
...
There are `r length(unique(nodes$dataset))` datasets. A gene that is perfectly sampled would form a cluster with this size. Very few clusters, though, are this size. This suggests that intersection of genes between matrices is low
